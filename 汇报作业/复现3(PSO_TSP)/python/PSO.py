...
class PSO(object):
    def __init__(self, num_city, data):
        ...
        # 初始化所有粒子
        # 随机初始化
        # self.particals = self.random_init(self.dis_mat,num_total=self.num,num_city =num_city)
        # 贪心初始化
    ...
    def cross(self, cur, best):
        ...
        t = np.random.choice(l,2) #随机选择交叉点
        x = min(t)
        y = max(t)
        cross_part = best[x:y]
        tmp = []
        ...
        for t in one:
            if t in cross_part:
                continue
            tmp.append(t) # 交叉点之外的城市构成部分路径
        # 两种交叉方法
        one = tmp + cross_part # 交叉点之外的城市在前，交叉点之内的城市在后
        l1 = self.compute_pathlen(one, self.dis_mat)
        one2 = cross_part + tmp # 交叉点之内的城市在前，交叉点之外的城市在后
        ...
        for cnt in range(1, self.iter_max):
            ...
            for i, one in enumerate(self.particals):
                ...
                if new_l < self.best_l:
                    self.best_l = tmp_l
                    self.best_path = one
                # 新个体更优则新最优与全局最优交叉；否则一定概率选择新旧个体交叉
                ...
                # 新个体更优则选择新个体；否则一定概率选择新旧个体  作为粒子新位置
                if new_l < tmp_l or np.random.rand()<0.1:
                    one = new_one
                    tmp_l = new_l
